...
inline void clear(int x){
	FOR(i,(x-1)*BLO+1,x*BLO) add(g[x][val[i]],ha-f[i-1]);
}

inline void build(int x){
	// FOR(i,0,2) assert(g[x][i]==0);
	FOR(i,(x-1)*BLO+1,x*BLO) val[i] += tag[x];
	tag[x] = 0;sm[x] = 0;
...
inline void modify(int l,int r,int d){ // [0..n]
	...
	clear(bel[l]);FOR(i,l,bel[l]*BLO) val[i] += d;build(bel[l]);
	FOR(i,bel[l]+1,bel[r]-1){
		if(d == 1) {if(k-tag[i] >= 0) add(sm[i],ha-g[i][k-tag[i]]);}
		else {if(k-tag[i]+1 >= 0) add(sm[i],g[i][k-tag[i]+1]);}
		tag[i] += d;
	}
	clear(bel[r]);FOR(i,(bel[r]-1)*BLO+1,r) val[i] += d;build(bel[r]);
}

inline void upd(int p,int x){
	clear(bel[p+1]);build(bel[p+1]);f[p] = x;
