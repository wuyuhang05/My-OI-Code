...
int main(){
	...
	FOR(i,1,k){
		FOR(x,1,n){
			FOR(y,1,m){
				...
				tmp.pb(Node(n*m+id(x,y),2*n*m+k+i,1,d));
			}
		}
		FOR(x,1,n){
			FOR(y,1,m){
				...
				tmp.pb(Node(2*n*m+i,id(x,y),1,d));
			}
		}
	}
	std::sort(all(tmp));
	int q = std::sqrt(1.0*tmp.size()),c = 1;
	for(int l = 0,r = q-1;l < tmp.size();l = r+1,++c){
		r = std::min((int)tmp.size()-1,l+q-1);
		FOR(i,l,r) edge[c].pb(tmp[i]);
	}
	// DEBUG(F::S);
	int ps = -1;
	// exit(0);
	FOR(i,1,c){
		for(auto x:edge[i]) F::add(x.x,x.y,x.z);
		F::Dinic();
		if(F::ans == k){
			ps = i;break;
		}
	...
	FOR(i,1,k) F::add(F::S,2*n*m+i,1),F::add(2*n*m+k+i,F::T,1);
	FOR(i,1,n) FOR(j,1,m) F::add(id(i,j),n*m+id(i,j),1);
	FOR(i,1,ps-1) for(auto x:edge[i]) F::add(x.x,x.y,x.z);
	std::sort(all(edge[ps]));
	LL ans = -1;
	for(auto x:edge[ps]){
		F::add(x.x,x.y,x.z);F::Dinic();
		if(F::ans == k){ans = x.t;break;}
	}
	printf("%lld\n",ans);
	return 0;
}