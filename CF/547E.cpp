/*
把这些串用特殊字符连接拼起来之后搞个 SA
首先我们定义到k 字符串的 rk 的位置,发现可以找出一段区间,满足这个区间内任意一个后缀和 k字符串代表的后缀的 lcp >= len[k]
可以表示 rk 在这个范围([L,R])内的串都满足出现了这个字符串
然后我们对于每个后缀 用它最后一个字母所在的字符串的编号来宝石
于是还是要查询编号在 [l,r] 范围的.
就是一个二维数点问题了.可以离线下来树状数组做,也可以主席树在线做
 */
#include <algorithm>
#include <iostream>
#include <cstring>
