/*
考虑如果 l~r 非常短的话可以直接全部插进去构个 AC 自动机然后数位 dp
考虑我们现在暴力构造的话 trie 数很有可能有一堆 10 叉树
我们类似数位 dp,将所有数"刚刚突破边界"的字符串插入trie,标志着到达这个点后如果后面能再走一些步数就能获得这个点的贡献
所以我们设 g[i][j] 表示到达 i 这个点 再走 j 步对答案的贡献
f[i][j] 表示填到 i 位 在 j 这个点上
f[i][j]*g[to][1~n-i-1] -> f[i+1][to]
 */
#include <algorithm>
#include <iostream>
#include <cstring>
