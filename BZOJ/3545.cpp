...
inline void add(int u,int v){
    ee[++ccnt] = (EEdge){v,head[u]};head[u] = ccnt;
    ee[++ccnt] = (EEdge){u,head[v]};head[v] = ccnt;
}
...
int main(){
    ...
    FOR(i,1,n) read(h[i]),b[++szz] = h[i];
    std::sort(b+1,b+szz+1);
    szz = std::unique(b+1,b+szz+1)-b-1;
//    DEBUG(szz);
    FOR(i,1,n) h[i] = std::lower_bound(b+1,b+szz+1,h[i])-b;
    ...
    FOR(i,1,q){
        ...
        v ^= la;x ^= la;k ^= la;
        int rt = getfa(v,x);
        int len = sm[root[dfn[rt]+sz[rt]-1]]-sm[root[dfn[rt]]];
        if(len < k) puts("-1"),la = 0;
        else printf("%d\n",la = b[query(root[dfn[rt]],root[dfn[rt]+sz[rt]-1],1,n,len-k+1)]);
    }
    return 0;
}
