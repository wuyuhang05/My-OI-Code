...
const int prime[] = {0,2,3,7,19260817};
...
int p;
std::vector<int> Div;
std::vector<P> res,rres;

inline void Solve(){
	scanf("%lld",&p);res.clear();
	FOR(k,1,lim){
		LL t = p*k+1;ans.clear();work(t,19260817);
		Div.clear();Div.pb(1);int tt = t;
		...
		FOR(i,1,(int)Div.size()-1) if(Div[i] < p && t/Div[i] < p) res.pb(MP(Div[i],t/Div[i]));
	}
	std::sort(all(res));rres.clear();int mn = p;
	FOR(i,0,(int)res.size()-1){
		if(i != 0 && res[i-1].fi == res[i].fi) continue;
		if(mn > res[i].se) rres.pb(MP(res[i].fi,res[i].se)),mn = res[i].se;
	}
	printf("%lu\n",rres.size());
	for(auto x:rres) printf("%lld %lld\n",x.fi,x.se);
}
